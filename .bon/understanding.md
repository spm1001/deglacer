...
**deja** (github.com/vshulcz/deja-vu) was benched on 2026-08-09 and routed to from the trousse skill as the ranked whole-history search. Measured on 2026-09-13: 52 invocations ever in 21 sessions, two genuine searches by a Claude in the preceding month, index last written 2026-09-02, installed build pinned at v0.16.9 against an upstream at v0.20.0 that ships every two to three days and is heading toward auto-recall inside agents — the surface the adoption card ruled out. Meanwhile Claudes ran 192 cross-session `rg` searches over the transcripts in the same period. The recall need is real; deja is not what meets it. Recommendation on dgc-secise: drop it, and build `--index` / `--search` on SQLite FTS5 over conversation text only, benched against the existing banc rig before the skill's routing changes. Sameer decides.
