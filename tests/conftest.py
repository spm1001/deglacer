...
def _hermetic_invocation_log(tmp_path_factory, monkeypatch):
    ...
    into the real ~/.local/share/deglacer/invocations.jsonl (the _invlog
    shim's output is analysis data — test invocations would poison the
    caller-stamp baseline). The CLI smoke tests run the CLI as a real
    subprocess with env built from os.environ, so this propagates there too.
    Tests that probe the log override XDG_DATA_HOME themselves."""
